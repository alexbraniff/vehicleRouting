...
public class VRPTW {
  ...
  public VRPTW(int heuristicMode, String dataFile, boolean isForSolutions)
  {
	  ...
	  case 3:
		  ...
		  // set up the shipment insertion type
		  problemInfo.setInsertionHeuristic(new SolomonsSequentialInsert(1, 0, 1, 1));
  ...
  public void createInitialRoutes()
  {
	  ...
	  case 3:
	      ...
	      if(unrouted.size() > 0)
	      {
	    	  ...
	    	  {
	    		  VRPTWTruck newTruck = new VRPTWTruck(this.problemInfo.getTruckType(), (int) mainDepots.getHead().getNext().getXCoord(), (int) mainDepots.getHead().getNext().getYCoord());
	    		  newTruck.getMainNodes().insertLastNode(new VRPTWNodes((VRPTWShipment) unrouted.elementAt(i)));
	    		  unrouted.removeElementAt(i);
	    		  mainDepots.getHead().getNext().getMainTrucks().insertTruckLast(newTruck);
	    	  }
	    	  
	    	  mainDepots.linkRoutes(problemInfo);
