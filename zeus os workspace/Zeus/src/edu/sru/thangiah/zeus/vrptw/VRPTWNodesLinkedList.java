								 ...
								 VRPTWShipment theShipment)
{
	...
	while(i != (VRPTWNodes) currNodeLL.getTail())
	{
		...
		if(cBest == null)
		{
			cBestNode = i;
			cBest = c2;
			//System.out.println("SIH: Initial lowest cost is " + cBest + " (i:" + cBestNode.getShipment().getIndex() + ", j:" + j.getShipment().getIndex() + ")");
		}
		else
		{
			if(Math.abs(c2) < Math.abs(cBest))
			{
				cBestNode = i;
				cBest = c2;
				//System.out.println("SIH: New lowest cost is " + cBest + " (i:" + cBestNode.getShipment().getIndex() + ", j:" + j.getShipment().getIndex() + ")");
			}
		}
	...
	if(cBest != null)
	{
		...
		//System.out.println("SIH: Best insertion after " + cBestNode.getShipment().getIndex() + " and before " + cBestNode.getNext().getShipment().getIndex());
		
		currNodeLL.insertAfterNodes(theNode, cBestNode);
		ProblemInfo.nodesLLLevelCostF.calculateTotalsStats(currNodeLL);
