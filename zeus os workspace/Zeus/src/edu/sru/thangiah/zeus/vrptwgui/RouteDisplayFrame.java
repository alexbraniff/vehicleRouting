package edu.sru.thangiah.zeus.vrptwgui;
  ...
  public class ZRDMouseMotionListener
      ...
      public void mouseMoved(MouseEvent evt) {
        ...
        if(shipIndex != -1) {
          ...
          VRPTWNodes node = getNodeContainingShipment(shipIndex);
          
          jW.setText("<html>"
        		     + "<h2>#" + shipIndex + " Demand: " + getShipDemand(shipIndex) + "</h2>"
				     + "<ul>"
    		           + "<li>Serv. Time: " + node.getServiceTime() + "</li>"
				       + "<li>Early: " + node.getEarly() + "</li>"
				       + "<li>Late: " + node.getLate() + "</li>"
				       + "<li>Begin: " + node.getBeginTime() + "</li>"
				     + "</ul>"
				   + "</html>");
          ...
          double jlabelWidth = jW.getPreferredSize().getWidth();
          double jlabelHeight = jW.getPreferredSize().getHeight();
          
          double jlabelx = evt.getPoint().getX();
          double jlabely = evt.getPoint().getY();

          if(jlabelx + jlabelWidth > RDFPanel.getSize().getWidth())
          {
          	jlabelx -= jlabelWidth;
          }
          
          if(jlabely + jlabelHeight > RDFPanel.getSize().getHeight())
          {
          	jlabely -= jlabelHeight;
          }
          
          panelLayout.addConstraint("JW", AttributeType.TOP, new AttributeConstraint(DependencyManager.ROOT_NAME, AttributeType.TOP, (int) jlabely + 10));
          panelLayout.addConstraint("JW", AttributeType.LEFT, new AttributeConstraint(DependencyManager.ROOT_NAME, AttributeType.LEFT, (int) jlabelx));
          RDFPanel.repaint();
        }
        if(shipIndex == -1) {
          RDFPanel.remove(jW);
    ...
    private double getShipDemand(int shipIndex) {
  	...
  	private VRPTWNodes getNodeContainingShipment(int shipIndex)
  	{
  		VRPTWDepot d = (VRPTWDepot) ZeusGuiInfo.mainDepots.getHead().getNext();
  		while(d != (VRPTWDepot) ZeusGuiInfo.mainDepots.getTail())
  		{
  			VRPTWTruckLinkedList truckLL = (VRPTWTruckLinkedList) d.getMainTrucks();
  			VRPTWTruck t = (VRPTWTruck) truckLL.getHead().getNext();
  			while(t != (VRPTWTruck) truckLL.getTail())
  			{
  				VRPTWNodesLinkedList nodeLL = (VRPTWNodesLinkedList) t.getMainNodes();
  				VRPTWNodes n = (VRPTWNodes) nodeLL.getHead().getNext();
  				while(n != (VRPTWNodes) nodeLL.getTail())
  				{
  					if(n.getShipment().getIndex() == shipIndex)
  					{
  						return n;
  					}
  					
  					n = (VRPTWNodes) n.getNext();
  				}
