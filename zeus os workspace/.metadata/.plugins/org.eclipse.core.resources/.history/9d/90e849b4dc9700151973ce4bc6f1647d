...
/**
  ...
  {
		VRPTWShipment ship = (VRPTWShipment) this.getHead().getNext();
		out.println(this.getNumShipments());
		while (ship != (VRPTWShipment) this.getTail()) {
			out.println((int) ship.getIndex() + " " +
					(int) ship.getDemand() + " " +
					(int) ship.getEarly() + " " +
					(int) ship.getLate() + " " +
					(int) ship.getDuration() + " " + 
					(int) ship.getXCoord() + " " +
  ...
  public VRPTWShipment getNextInsertShipment(VRPTWProblemInfo problemInfo,
	  ...
	  case 2:
		  break;
	  case 3:
		  if(!problemInfo.getHasStartedRoutingTheUnrouted())
		  {
			  theShip = ((SmallestPolarAngleToDepot) problemInfo.getSelectionHeuristic()).getSelectShipment(currDepotLL, currDepot, currShipmentLL,currShip);
		  }
		  else
...
extends VRPTWShipmentLinkedList {
  ...
  while(temp != (VRPTWShipment) currShipLL.getTail())
  {
     	//System.out.println("ShipmentLL: Testing " + temp.getIndex());
	    ...
	    double be = temp.getEarly();
	    double bl = temp.getLate();
        ...
        //System.out.println("ShipmentLL: a(" + ax + ", " + ay + "), b(" + bx + ", " + by + ")");
	    
        //System.out.println("ShipmentLL: a(e:" + ae + ", l:" + al + ", b:" + iBegin + "), b(e:" + be + ", l:" + bl + ", b:" + jBegin + "), cij = " + cij);
	    ...
	    if(jBegin <= temp.getLate())
	    {
		    if(bestCost == null)
		    {
		    	bestShipment = temp;
		    	bestCost = cij;
		        //System.out.println("ShipmentLL: Initial best cost is " + bestCost + " and best shipment is " + temp.getIndex());
		    }
		    else if(cij < bestCost)
		    {
		    	bestShipment = temp;
		    	bestCost = cij;
		        //System.out.println("ShipmentLL: New best cost is " + bestCost + " and best shipment is " + temp.getIndex());
		    }
	    }
	    else
	    {
