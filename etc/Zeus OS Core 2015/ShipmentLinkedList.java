...
public class ShipmentLinkedList {
	...
	public boolean insertLast(Shipment shipment) 
	{
		...
		shipment.setIndex(numShipments);
		totalDemand+=shipment.getDemand(); //get the total number of students per pickup point
		return true; 
	}
	
	/**
	 * removes the last student in the shipment linked list.
	 */
	public Shipment removeLast() 
	{
		Shipment ptr;
		...
		ptr = tail.getPrev();
		ptr.getPrev().setNext(tail);
		tail.setPrev(ptr.getPrev());
		ptr.setPrev(null);
		ptr.setNext(null);
		numShipments++; //increment number of pickup points
		totalDemand -= ptr.getDemand(); //get the total number of students per pickup point
	...
	/**
	 * Returns if all shipments have been selected  or not
	...
	/**
	 * Returns the total number of shipments that have been assigned
	 */
	public int getTotalShipsAssigned() {
		Shipment ship = head.getNext();
