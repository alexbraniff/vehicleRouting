...
public class NodesLinkedList
  ...
  private int noNodes; //number of customers or shipments in the NodeLinkedList
...
/**
  ...
  public Nodes getNodesAtPosition(int pos) {
    ...
    while ( (current != tail) && (currentPos < pos)) {
  ...
  public String getRouteString() {
    ...
    Nodes node = this.getHead().getNext();
    while (node != tail) {
...
/**
      ...
      while (tempCell != tail) { //loop through all the cells until last depot
        ...
        if (!tempCell.getPickupPointName().equals(pupName)) {
          ...
          while ( (tempCell.getNext() != tail) &&
            ...
            tempDemand += tempCell.getShipment().getDemand();
            tempCell = tempCell.getNext();
          }
          ...
          firstCellInPUP.getShipment().setTempNext(firstCellInPUP.getNext()); //connect cells in this PUP to tempNext
          firstCellInPUP.getShipment().setTempDemand(firstCellInPUP.getShipment().getDemand());
              ;
          firstCellInPUP.getShipment().setDemand(tempDemand);
          ...
          if (firstCellInPUP != lastCellInPUP) {
            ...
            firstCellInPUP.getShipment().setTempNext(null); //only cell from this PUP, nothing to save
          }
  ...
  public void expandRoute() {
    ...
    try {
      while (tempCell != tail) {
        if (tempCell.getShipment().getTempNext() != null) { //there are hidden cells
          startOfHidden = tempCell.getShipment().getTempNext();
          ...
          tempCell.setNext(tempCell.getShipment().getTempNext()); //reattach the first hidden cell
          tempCell.getShipment().setTempNext(null);
          tempCell.getShipment().setDemand(tempCell.getShipment().getTempDemand());
          tempCell.getShipment().setTempDemand(-1);
